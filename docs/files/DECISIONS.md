...
**Decision (approved 2026-09-11; recovery behavior updated 2026-09-13):** When `SERP_API_KEY` is configured, the verified market-data pipeline uses SerpAPI's eBay sold endpoint (`engine=ebay&show_only=Sold`) as the primary sold-history source. Operational failures use the explicit server-side chain `SerpAPI → SoldComps → Trawl`; every provider attempt, failure reason, and latency is retained in the scan audit. A successful empty response is evidence, not an outage, and does not trigger another provider.
**Why:** `SERP_API_KEY` is already configured in Supabase secrets for visual identification (Google Lens). `show_only=Sold` returns only verified completed sales (not unsold completed listings). A `sold_date`-absent result is still admitted — the sold filter guarantees the item sold; the exact date is display-only. Production scan 78 proved why audited failover is required: SerpAPI returned an operational 503 for the exact-model query, SoldComps returned 40 real records, six exact-model comps qualified, and the scanner produced an evidence-backed result instead of `LIMITED EVIDENCE`.
**Do not:** expose any provider key client-side, call sold-data providers from `app.html`, use results outside the existing comp-matching pipeline, treat `show_only=Complete` results as sold evidence, hide a provider switch from the audit, or convert provider errors into a sourcing decision.
